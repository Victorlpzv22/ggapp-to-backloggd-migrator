...
import {
  buildSlugVariants, buildCleanSlug, normalizeForSearch,
  stripTrademarks, normalizeApostrophes, stripDuplicateTag,
  normalizeForMatch, normalizeForMatchExtended, slugToDisplayName,
} from './slug';
...
describe('slug utils', () => {
  ...
  it('buildCleanSlug default removes apostrophe (regex char-class), stripApos=true gives different output', () => {
    ...
    expect(buildCleanSlug("Assassin's Creed II", true)).not.toBe(buildCleanSlug("Assassin's Creed II"));
  });
  it('buildSlugVariants with originalSlug: original first, then clean, then arabic', () => {
    expect(buildSlugVariants('Hellblade II', 'hellblade-2')).toEqual(['hellblade-2', 'hellblade-ii']);
  });
